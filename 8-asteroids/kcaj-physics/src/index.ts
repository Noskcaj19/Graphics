...
export class ContactPoint {
    ...
    constructor(a: PhysicsObject, b: PhysicsObject, localA: Vector3, localB: Vector3, normal: Vector3, penetration: number) {
        this.a = a
        this.b = b
        this.localA = localA
        this.localB = localB
...
export class Transform {
    ...
    updateMatrices() {
        ...
        mat4.multiply(this.localMatrix, this.localMatrix, mat4.fromQuat(mat4.create(), this.localOrientation))
        mat4.multiply(this.localMatrix, this.localMatrix, mat4.fromScaling(mat4.create(), this.localScale))
...
export class PhysicsSystem {
    ...
    impulseResolveCollision(a: PhysicsObject, b: PhysicsObject, p: ContactPoint) {
        ...
        let pNS2 = vec3.scale(pNormalScaled, pNormalScaled, a.inverseMass / totalMass)
        vec3.subtract(transformA.worldPosition, transformA.worldPosition, pNS2)
        vec3.add(transformB.worldPosition, transformB.worldPosition, pNS2)
...
namespace Collision {
    ...
    function AABBSphereIntersection(
        ...
        let closestPointOnBox = clampVec3(delta, boxSize.scaled(-1), boxSize)
        // let localPoint = delta.minused(closestPointOnBox)
        delta.minus(closestPointOnBox)
        let localPoint = delta
        let dist = localPoint.length()

        if (dist < volumeB.radius) {
            let normal = localPoint.normalized()
...
function clampVec3(a: Vector3, min: Vector3, max: Vector3): Vector3 {
    return new Vector3(
