...
export class PhysicsSystem {
    ...
    basicCollisionDetection() {
        ...
        for (let i = 0; i < this.objects.length; i++) {
            if (!this.objects[i].collide) continue
...
namespace Collision {
    ...
    function AABBSphereIntersection(
        ...
        let delta = vec3.subtract(vec3.create(), transformB.worldPosition, transformA.worldPosition)

        let closestPointOnBox = clampVec3(delta, vec3.scale(vec3.create(), boxSize, -1), boxSize)
        // let localPoint = delta.minused(closestPointOnBox)
        let localPoint = vec3.subtract(vec3.create(), delta, closestPointOnBox)
        ...
        if (dist < volumeB.radius) {
            ...
            let lB = vec3.scale(vec3.create(), normal, -1)
            lB = vec3.scale(vec3.create(), lB, volumeB.radius)
            // let lB = normal.scaled(-1).scaled(volumeB.radius)
